<!-- php header code  -->
    <?php include "header.php"; ?>


  <!-- Start main-content -->
  <div class="main-content">
    <!-- Section: inner-header -->
    <section class="inner-header parallax layer-overlay" data-bg-img="images/bg/bg5.jpg">
      <div class="container pt-140 pb-50">
        <!-- Section Content -->
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-12 xs-text-center">
              <h1 class="font-32 text-theme-colored">Decorative PVC Wall Panels</h1>
             
            </div>
          </div>
        </div>
      </div>
    </section>

   <section>
      <div class="container mt-30 mb-30 pt-30 pb-30">
        <div class="row">
          <div class="col-md-8 blog-pull-right flip">
            <!-- Section: About -->
            <section>
              <div class="row">
                <div class="col-md-12">
                  <h3 class="text-theme-colored">Decorative PVC Wall Panels</h3>
                  <p class="text-black">
<strong>
PVC ceiling and wall panels are easy to install, water-proof, maintenance-free and is available in various plain colours and prelam wood veneer designs. </strong></p>
                 
              
                  <img class="img-fullwidth" src="images/services/decorative.jpg" alt=""><br><br>
               
				<p class="text-black">PVC wall paneling enhances the look of the interior environments of homes, hotels, offices, hospitals, computer labs etc.   </p>
				
				<h4 class="text-theme-colored">Features</h4>
				<ul class="list theme-colored angle-double-right">
				<li> Fire proof, rot proof , damp proof and rust proof</li>
				<li>Water proof and heat proof </li>
				<li> Easy maintenance</li>
				<li> Sound Resistant</li>
				<li> Impact Resistant</li>
				<li> Light in weight and easy to install can save a lot of time and man power cost.</li>
				<li> Easy to cut, drilled, nailed and sawed. PVC Wall Ceiling Panel</li>
				
				</ul>
				
				
				
				
				
				
              </div>
            </section>
          </div>
          <div class="col-sm-12 col-md-4">
            <div class="sidebar sidebar-left mt-sm-30">
              <div class="widget">
                <h5 class="widget-title line-bottom">Our Serivces</h5>
                <div class="categories">
                  <ul class="list list-border angle-double-right">
                    <li>
                                                        <a href="interior-paint-finish.php"><strong>Interior Paint Finish</a>
                                                    </li>
                                                    <li>
                                                        <a href="exterior-paint-finish.php">Exterior Paint Finish</a>
                                                    </li>
                                                    <li>
                                                        <a href="water-proofing-solution.php">Water Proofing Solution</a>
                                                    </li>
													<li>
                                                        <a href="wooden-coating-solution.php">Wooden Coating Solution</a>
                                                    </li>
													<li>
                                                        <a href="interior-designing.php">Interior Designing</a>
                                                    </li>
                                                    <li>
                                                        <a href="exterior-designing.php">Exterior Designing</a>
                                                    </li>
													<li>
                                                        <a href="modular-kitchen.php">Modular Kitchen</a>
                                                    </li>
													<li>
                                                        <a href="decorative-pvc-wall-panels.php">Decorative PVC Wall panels</strong></a>
                                                    </li>
                  </ul>
                </div>
              </div>
              <div class="widget">
                <div class="fb-page">
                  <h5 class="text-theme-colored"> Latest Facebook Updates</h5>
                  <iframe src="https://www.faceboo/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2Furbanindiadesign&amp;tabs=timeline&amp;width=340&amp;height=500&amp;small_header=false&amp;adapt_container_width=true&amp;hide_cover=false&amp;show_facepile=true&amp;appId" width="330" height="500" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>
          
                </div>
              </div>		
              
             
            </div>
          </div>
        </div>
      </div>
    </section>




   
  </div>
  <!-- end main-content -->
  
<!-- php footer code  -->
    <?php include "footer.php"; ?>

    <script src="js/logo.js"></script>