<!-- php header code  -->
    <?php include "header.php"; ?>


  <!-- Start main-content -->
  <div class="main-content">
    <!-- Section: inner-header -->
    <section class="inner-header parallax layer-overlay" data-bg-img="images/bg/bg5.jpg">
      <div class="container pt-140 pb-50">
        <!-- Section Content -->
        <div class="section-content text-center">
          <div class="row"> 
            <div class="col-md-12 xs-text-center">
              <h1 class="font-32 text-theme-colored">Interior Paint Finish</h1>
             
            </div>
          </div>
        </div>
      </div>
    </section>

   <section>
      <div class="container mt-30 mb-30 pt-30 pb-30">
        <div class="row">
          <div class="col-md-8 blog-pull-right flip">
            <!-- Section: About -->
            <section>
              <div class="row">
                <div class="col-md-12">
                  <h3>Interior Paint Finish</h3>
                  <p class="text-black">

To choose which paint is best for your project? Do the research. The type of paint you choose is just as important as the color, and can have a major effect on the character of your room and the vibe you want to achieve. WATER-BASED PAINTS The majority of paints for wall sold today is water-based, primarily because of its easiness of use.</p>
                 
              
                  <img class="img-fullwidth" src="images/services/interior.jpg" alt=""><br><br>
               
				<p class="text-black">If your surface has been previously coated with an oil-based product, be cautious when switching to water-based paint as it may have trouble sticking. In this situation, washing the surface is recommended and then rubbing it all over with a medium to smooth sandpaper—making it clean, dry, and dull in order to prevent peeling of the new coat</p>
				
				
              </div>
            </section>
          </div>
          <div class="col-sm-12 col-md-4">
            <div class="sidebar sidebar-left mt-sm-30">
              <div class="widget">
                <h5 class="widget-title line-bottom">Our Serivces</h5>
                <div class="categories">
                  <ul class="list list-border angle-double-right">
                    <li>
                                                        <a href="interior-paint-finish.php"><strong>Interior Paint Finish</a>
                                                    </li>
                                                    <li>
                                                        <a href="exterior-paint-finish.php">Exterior Paint Finish</a>
                                                    </li>
                                                    <li>
                                                        <a href="water-proofing-solution.php">Water Proofing Solution</a>
                                                    </li>
													<li>
                                                        <a href="wooden-coating-solution.php">Wooden Coating Solution</a>
                                                    </li>
													<li>
                                                        <a href="interior-designing.php">Interior Designing</a>
                                                    </li>
                                                    <li>
                                                        <a href="exterior-designing.php">Exterior Designing</a>
                                                    </li>
													<li>
                                                        <a href="modular-kitchen.php">Modular Kitchen</a>
                                                    </li>
													<li>
                                                        <a href="decorative-pvc-wall-panels.php">Decorative PVC Wall panels</strong></a>
                                                    </li>
                  </ul>
                </div>
              </div>
              <div class="widget">
                <div class="fb-page">
                  <h5 class="text-theme-colored"> Latest Facebook Updates</h5>
                  <iframe src="https://www.faceboo/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2Furbanindiadesign&amp;tabs=timeline&amp;width=340&amp;height=500&amp;small_header=false&amp;adapt_container_width=true&amp;hide_cover=false&amp;show_facepile=true&amp;appId" width="330" height="500" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowTransparency="true" allow="encrypted-media"></iframe>
          
                </div>
              </div>		
              
             
            </div>
          </div>
        </div>
      </div>
    </section>




   
  </div>
  <!-- end main-content -->
  
  <!-- php footer code  -->
    <?php include "footer.php"; ?>